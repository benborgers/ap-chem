...
**Lattice energy** is the change in energy when ions in their gaseous state form an ionic solid. It is exothermic, since creating an ionic compound releases energy. 
...
It is very difficult to directly measure lattice energies for ionic substances, so they are usually theoretically calcuated from other data. The other changes add up to the full change from gaseous ions to ionic solid, and their energies can be added together (kind of like Hess's Law). 

*Dr. Barchuk said that we don't need to memorize the exact steps within calculating lattice energy.*

<Callout>
  A higher melting point is also associated with a higher lattice energy. 
</Callout>
...
*One would actually have to draw four resonant structures with five bonds on the phosphorus atom, making four structures similar to the rightmost structure above. This is because there are four ways to place the double bond.*


# Electronegativity

**Electronegativity** is the ability of an atom in a molecule to attract atoms to itself. On the periodic table, electronegativity generally increases from left to right and bottom to top. 
...
Electrons tend to spend more time around the more electronegative atom. The result is a partial negative charge (*not* a complete transfer of charge). It is represented by <InlineMath text="\delta +" /> ("partially negative") or <InlineMath text="\delta -" /> ("partially negative").
