...
1. Sum the valence electrons from all atoms, taking into account overall charge. 
  ...
  <InlineMath text="\text{P}" /> has 5 electrons, and each <InlineMath text="\text{Cl}" /> has 7 electrons, so there are 26 total electrons. 
...
For example, in <InlineMath text="\text{PO}_4 \, ^\text{3-}" /> [O's oxidation number is -2](/unit-4/#oxidation-numbers). Since the overall charge of the molecule should 3-, the charge of P is +5. Therefore, P should give away 5 electrons and will have 5 covalent bonds. 
...
* In some situations (such as the second example for 3 electron domains above), the angles may become lesser when bonds are replaced with lone pairs. This is also because lone pairs create more repulsion than bonds and take up more room than bonds, so they push on the remaining bonds. 
